...
The user's own dev servers are usually running — kzen-auto on `127.0.0.1:8080` (`BackendDevelopment`) and often an interactive tester on `18081`. Never kill or reuse them (see the umbrella [`../kzen/AGENTS.md`](../kzen/AGENTS.md) rule); boot your own instance on a spare port, and before stopping any JVM verify its `CommandLine` contains the `--server.port=` you chose. Run built jars with an explicit Java 26 (`~/.jdks/temurin-26.0.1` — PATH `java` is often 8).
