...
### Field copy is declared in notation: `meta.<attr>.label` / `.description` / `.details`

An attribute's user-facing wording lives beside its type in the archetype, not in the editor: `label:` replaces the name-derived default (`CommonEditUtils.formattedLabel`, which otherwise splits camelCase — `schemaMode` → "Schema Mode"), `description:` is one plain-language line `AttributeEditorManager` draws beneath whichever editor the field resolves to, and `details:` (a sibling of `values:`, same keys) gives `SelectValuesEditor` a second line per option. All three are optional, stored verbatim by kzen-lib, and written for someone who has never seen the source (CC-18) — the `File` source's `DataReadWorkerConfig` / `FileDataSourceConfig` are the worked example. The project's YAML parser rejects backslash escapes such as `\d` in these strings, quoted either way.
