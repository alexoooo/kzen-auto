...
These files are **auto-generated by `ModuleReflectionGenerator`** — do not hand-edit. The generator itself lives in **kzen-lib** (`kzen-lib-jvm/src/main/kotlin/tech/kzen/lib/server/codegen/ModuleReflectionGenerator.kt`), not in kzen-auto; kzen-auto consumes it. Re-running the generator picks up new SPI classes by reflection and rewrites the file.

Run via Gradle (tasks registered in `kzen-auto-jvm/build.gradle.kts`, group `codegen`):

```powershell
./gradlew :kzen-auto-jvm:runAllCodegen      # all three module files
./gradlew :kzen-auto-jvm:runCommonCodegen   # KzenAutoCommonModule.kt only
./gradlew :kzen-auto-jvm:runJvmCodegen      # KzenAutoJvmModule.kt only
./gradlew :kzen-auto-jvm:runJsCodegen       # KzenAutoJsModule.kt only
```

Each task is a `JavaExec` wrapping the corresponding `tech.kzen.auto.server.codegen.KzenAuto*Codegen` main from `src/test/kotlin` (test runtime classpath). Working directory is pinned to `rootProject.rootDir` because the generator resolves output paths relative to the kzen-auto root.
...
2. Run `./gradlew :kzen-auto-jvm:runAllCodegen` (or the targeted task for just the module that changed).
