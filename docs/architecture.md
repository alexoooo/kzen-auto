...
Clients must key re-fetch on **`ClientLogicState.traceVersion()`** — one shared rule (`epoch|runId|sequence|state`), never a per-flavour notion of "changed", and never a timestamp. Both `Long`s serialize as **strings**, following the existing `LogicTraceEvent.sequence` convention that dodges JS `Long` round-tripping. Live trace views (`lookup` / `lookupRun`) remain full snapshots — they are *sequence-gated* (not fetched unless the run moved), not incremental; making them delta-fetched would need engine-side reset tombstones, since `resetEmitted` clears live values a delta pass would miss and ghost.
...
- **Structure changed** ⇒ publish immediately, and reset the throttle's clock. `ClientLogicState.structureVersion()` is `traceVersion` minus the sequence (`epoch|runId|state`): a run started, settled, changed state — **every step boundary is one of these** — or a trace was cleared. These are transitions the user is waiting on, so stepping stays instant and the epoch still drives the clear-repaint.
...
The deliberate cost: `structureVersion` **excludes the frame**, because a Script's frame position changes on essentially every step — folding it in would mark a plain run structure-changing throughout and defeat the throttle. So intermediate frames traversed *within* one step (a stepped-over `RunStep` descending into its child) repaint at the throttle's cadence, not per emit. Still animated: the step boundary resets the clock, so the first intermediate publishes at once. The remaining traffic (`lookup` + `lookupRunHistory`, both genuinely fresh per publish) is this design's floor; going below it needs a server-side structural version on `LogicStatus` (deferred — a kzen-lib wire change).
