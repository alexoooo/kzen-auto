...
**Ownership is offered by the binder, not claimed by an ancestor.** A Script document's **context signature** has two halves: `context: { exports: [...] }` lists the Contexts it offers upward to its caller, and `context: { requires: [...] }` names the ambient Contexts a caller must already have bound (reads walk self → root, like Kotlin `context(...)` receivers). A binding the opening document does not export is **private** to that document's frame, disposed at its settle per `closePolicy`; an exported one climbs one hop per `exports` declaration and rests at the first frame that does not export it onward, or at the run root. So a sub-script opens the SUT and the enclosing test owns it — because the sub-script offered it up and the test did not pass it on. Resolution happens at **bind time**, in one inverted walk as the registration is made, and matching is family-granular (the part of the key before `':'`, so one `sut` export carries `sut:main` and `sut:other` as independent registrations). Only `Script` carries a context signature, so a resource opened inside a hosted Flow or Job is permanently private. `closePolicy` is the disposal rule alone — `auto` / `manual` / `keepOnFailure`, with no scope dimension — and applies at the frame where the registration rests. A `manual` handle outlives that frame by a second, orthogonal mechanism: at the resting frame's settle the engine hands the registration up one level (cascading toward the run root), so a browser opened in one sub-script stays readable and explicitly closeable by later sibling sub-scripts (logic-spec §6); it is also the only way an un-exported resource survives the frame that bound it. Statically, a step whose `uses` nothing satisfies is a **blocking error** that greys Run out client-side — naming the document that binds the Context without exporting it, when that is the cause — while a binding nothing consumes draws no diagnostic at all. Open resources **survive live-edit migration**: the engine lifts registrations at the `migrate` barrier and re-adopts them by owning frame's stable identity (logic-spec §5), so pause → edit → resume keeps the same browser window; export declarations are not lifted, because each rebuilt `Logic.run` re-declares them.

`Context` is an ordinary notation archetype (`common-document.yaml`), so a plugin introduces its own with zero framework change: `is: Context`, plus `key` (the engine resource key, a **global namespace**), `class` (the value type), and title/icon/description. Three notation rules:

- **A concrete Context is `abstract: true`** (like `ResourceClosePolicy` and `TypeMetadata`) — the standard never-instantiated-archetype pattern. `class:` is kzen's *instantiation* key, so abstract is what lets it carry a type (`org.openqa.selenium.remote.RemoteWebDriver`) without `GraphCreator` constructing it.
- **The declarations that name a Context are weak references, declared `by: Nominal`** in their archetypes' `meta:` (`binds` on `ContextBinder`, `uses`/`releases` on `ScriptStep`, `context` on `Script`) — the same mechanism as `Custom.exports` and `RunStep.instructions`. So they are never constructor-injected, a dangling or abstract-targeted entry degrades to a validation warning instead of pruning the step from `transitiveSuccessful`, and renaming a Context rewrites them (`ContextRenameTest`). Blessed shapes: scalar for the single-valued `binds`/`releases`, list for `uses`/`context.exports`/`context.requires`; `LogicContextConventions` reads either. (Defence in depth: kzen-lib's `inferMetadata` also no longer promotes an *undeclared* scalar naming an `abstract: true` object — an inferred hard reference to one could only ever prune its host.)
- **A step that puts a value in scope mixes in `ContextBinder`**, and one that owns a disposable resource mixes in `ResourceOwner` (both `script-jvm.yaml`); `BrowserOpenStep` takes both. Binding and owning are separate declarations because they are separate facts — a step binding a String owns nothing, and a step deleting a temp file at settle binds nothing. A binder declares the Context it binds rather than calling `openResource` with a bare key: only the declaration is visible to the static analysis, so a bare open leaves every downstream `uses` unsatisfiable, and the only way to quiet the resulting errors is a `context.requires` on each calling document — which asserts that a caller supplies what nothing does. A declared binding can instead be exported, and the export states truthfully where the resource comes from.
- **Four generic steps make Contexts usable without writing a plugin** (`script/step/context/`, palette group "Context"). `BindStep` publishes a Kotlin expression's value under the Context it names — `is: [ScriptStep, ContextBinder]` and pointedly *not* `ResourceOwner`, so binding a String shows no close-policy control, not because an editor hides it but because there is nothing to hide. `UseContextStep` reads a Context back into the value graph, typed by the declaration's `type:`, so an ordinary `FormulaStep` can consume it by reference without knowing Contexts exist (`BrowserGetStep` is the hand-written case). `ReleaseStep` ends a Context; with no disposal attached it degenerates to unbinding, and there is deliberately no remove-without-dispose mode — that would have to be a separately named `unbind`, since the failure modes are opposite (a silent leak versus an early teardown). `DisposeAtSettleStep` wraps `disposeAtSettle` with no Context and no key, for cleanup nobody would want to read; its policy is `SettleDisposalPolicy`, which has two values rather than three because `manual` is a *promotion* and an anonymous registration has no name for anything to promote. All four take their Context through `SelectContextEditor`, sourced from `ContextConventions.allContexts` — and naming an `editor:` on a context declaration is itself the switch that makes it body-editable, so a typed step whose Context is an archetype constant (`BrowserOpenStep`) stays uneditable by naming none.
- **The typed bind surface is `StepExecution.bindContext`, in two shapes**: `bindContext(value, qualifier?)` binds with no disposal, `bindContext(value, closePolicy, qualifier?, closer)` attaches one. They share resolution, conformance checking and addressing and differ only in whether a disposal rides along — the `ContextBinder`/`ResourceOwner` split expressed at the runtime boundary. Conformance is centralized there rather than left to each binder, because a `String` bound to a Context contracted for `RemoteWebDriver` otherwise surfaces as a `ClassCastException` several steps away. `BindStep` adds the static half at definition time via `TypeAssignability`, and skips the class comparison when inference yields `Any` — inference does not distinguish a genuine `Any` from an unnameable classifier approximated to `Any`, so comparing against it would reject values whose static type generated Kotlin cannot import. Nullability survives the approximation and is checked either way.
...
| `context/` | Contexts | A user's own `is: Context` declarations — named slots in the ambient scope a run works against. The payload is a `by: NestedList` branch of real objects at `main.contexts/<Name>`: a declaration is a nominal symbol other documents name, so it needs an `ObjectLocation` for `by: Nominal` resolution, rename-as-refactor and picker discovery to reach it. Registers nothing — discovery is graph-wide by inheritance (`ContextConventions.allContexts`), so this is *an* authoring surface, not the home |
...
reflectionRegistry.put(
    "tech.kzen.auto.server.objects.data.schema.DataSchemaDocument",
    listOf("fields")
) { args ->
    tech.kzen.auto.server.objects.data.schema.DataSchemaDocument(args[0] as tech.kzen.auto.common.objects.document.data.schema.DataSchemaFieldListSpec)
}
```
