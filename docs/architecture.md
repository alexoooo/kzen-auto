...
The Selenium / WebDriver browser handle is **no longer a context service**: it is a per-run resource keyed `"browser"`, registered **with the engine** — the kzen-lib `RunEngine` stores the live handle (value) alongside the disposal closer on the owning node. Steps reach it through a declared **Context** rather than a bare string: `BrowserOpenStep` declares `provides: BrowserContext` and calls `StepExecution.provideContext(...)`, the action steps declare `requires: [BrowserContext]` and read `execution.context<RemoteWebDriver>()` (an ancestor-chain walk, so **any** hosted child — Script, Flow, or Job — reads the handle its host opened), and `BrowserCloseStep` declares `releases: BrowserContext`. The raw string-keyed API (`openResource` / `resource` / `releaseResource`) survives as the escape hatch beneath it; a Context's `key:` IS that string, so a typed step and a raw one naming `browser` share one registration. There is no Script-side value registry (the former `ScriptRunResources` was deleted when the engine took over value storage); `WebDriverSupport` holds only that shared key + the quiet-quit helper. This replaced the former `webDriverContext` process singleton (removes a global; allows concurrent runs).

**Ownership is offered by the provider, not claimed by an ancestor.** A Script document's **context signature** has two halves: `context: { exports: [...] }` lists the Contexts it offers upward to its caller, and `context: { requires: [...] }` names the ambient Contexts a caller must already have provided (reads walk self → root, like Kotlin `context(...)` receivers). A provide the opening document does not export is **private** to that document's frame, disposed at its settle per `closePolicy`; an exported one climbs one hop per `exports` declaration and rests at the first frame that does not export it onward, or at the run root. So a sub-script opens the SUT and the enclosing test owns it — because the sub-script offered it up and the test did not pass it on. Resolution happens at **bind time**, in one inverted walk as the registration is made, and matching is family-granular (the part of the key before `':'`, so one `sut` export carries `sut:main` and `sut:other` as independent registrations). Only `Script` carries a context signature, so a resource opened inside a hosted Flow or Job is permanently private. `closePolicy` is the disposal rule alone — `auto` / `manual` / `keepOnFailure`, with no scope dimension — and applies at the frame where the registration rests. A `manual` handle outlives that frame by a second, orthogonal mechanism: at the resting frame's settle the engine hands the registration up one level (cascading toward the run root), so a browser opened in one sub-script stays readable and explicitly closeable by later sibling sub-scripts (logic-spec §6); it is also the only way an un-exported resource survives its provider. Statically, a step whose `requires` nothing satisfies is a **blocking error** that greys Run out client-side — naming the document that provides the Context without exporting it, when that is the cause — while a provide nothing consumes draws no diagnostic at all. Open resources **survive live-edit migration**: the engine lifts registrations at the `migrate` barrier and re-adopts them by owning frame's stable identity (logic-spec §5), so pause → edit → resume keeps the same browser window; export declarations are not lifted, because each rebuilt `Logic.run` re-declares them.
...
- **The declarations that name a Context are weak references, declared `by: Nominal`** in their archetypes' `meta:` (`provides`/`requires`/`releases` on `ScriptStep`/`ContextProvider`, `context` on `Script`) — the same mechanism as `Custom.exports` and `RunStep.instructions`. So they are never constructor-injected, a dangling or abstract-targeted entry degrades to a validation warning instead of pruning the step from `transitiveSuccessful`, and renaming a Context rewrites them (`ContextRenameTest`). Blessed shapes: scalar for the single-valued `provides`/`releases`, list for `requires`/`context.exports`/`context.requires`; `LogicContextConventions` reads either. (Defence in depth: kzen-lib's `inferMetadata` also no longer promotes an *undeclared* scalar naming an `abstract: true` object — an inferred hard reference to one could only ever prune its host.)
- **A step that opens a run-scoped resource mixes in `ContextProvider`** (`script-jvm.yaml`), declaring the Context it provides rather than calling `openResource` with a bare key. Only the declaration is visible to the static analysis, so a bare open leaves every downstream `requires` unsatisfiable, and the only way to quiet the resulting errors is a `context.requires` on each calling document — which asserts that a caller supplies what nothing does. A declared provide can instead be exported, and the export states truthfully where the resource comes from.
