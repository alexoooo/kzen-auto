...
**Ownership is declared by the owner, not chosen by the opener.** A Script document declares `context: { slots: [...] }` for the Contexts it owns; a provide anywhere beneath it — including inside a hosted sub-Script — binds to the *nearest* declaring ancestor, falling back to the providing document when none declares one, and is disposed at that owner's settle per `closePolicy` (now just `auto` / `manual` / `keepOnFailure` — the disposal rule only; the flat `parent` / `run` variants and kzen-lib's `ResourceScope` are gone). So a sub-script opens the SUT and the enclosing test owns it, *because the test said so*. A `manual` handle outlives its owner's settle by a second, orthogonal mechanism — the engine hands the registration up one level (cascading toward the run root) — so a browser opened in one sub-script stays readable and explicitly closeable by later sibling sub-scripts (logic-spec §6). Open resources also **survive live-edit migration**: the engine lifts registrations at the `migrate` barrier and re-adopts them by owning frame's stable identity (logic-spec §5), so pause → edit → resume keeps the same browser window; slot declarations are not lifted, because each rebuilt `Logic.run` re-declares them.

`Context` is an ordinary notation archetype (`common-document.yaml`), so a plugin introduces its own with zero framework change: `is: Context`, plus `key` (the engine resource key, a **global namespace**), `class` (the value type), and title/icon/description. Two notation rules:
