 ...
 * the supplied baseUrl so requests ride the same kzen-shell reverse-proxy prefix as every other API
...
object IconLoader {
    ...
    // <Icon> renders — called synchronously from ClientContext construction.
    fun install(baseUrl: String) {
        setCustomIconsLoader(
            { names, _, _ -> async { fetch(baseUrl, names.toList()) } },
            IconNames.defaultSet)
    }

    // Best-effort startup preload; on failure the icons simply fetch lazily on first render.
    suspend fun preload(baseUrl: String) {
        try {
            addCollection(fetch(baseUrl, preloadNames))
        }
        catch (e: Throwable) {
    ...
    private suspend fun fetch(baseUrl: String, names: List<String>): dynamic {
