 ...
 * a notation document into an engine [tech.kzen.lib.common.exec.engine.Logic]. Bundled because all four
 * travel together through the Script and Flow compilers — and a [RunStep][tech.kzen.auto.server.exec.script.step.RunStep]
 * or [RunLogicVertex][tech.kzen.auto.server.objects.flow.vertex.RunLogicVertex] can nest the other flavour,
 * so each compiler must be able to compile an arbitrary child via [LogicCompiler] without re-threading args.
 *
