...
import tech.kzen.auto.server.objects.report.pipeline.event.v2.DisruptorPipelineOutput
import tech.kzen.auto.server.objects.report.pipeline.event.v2.ProcessorOutputEvent
import tech.kzen.auto.server.objects.report.pipeline.input.v2.instance.ProcessorDataInstance
...
//        private const val binaryDisruptorBufferSize = 512
    ...
    private fun setupBinaryDisruptor(
        ...
        if (charset != null) {
            ...
            binaryDisruptor.handleEventsWith(decoder::decodeDisruptor)
        }

        val framer = ProcessorInputFramer(processorDataInstance.dataFramer)
        binaryDisruptor.handleEventsWith(framer::frameDisruptor)

        val output = DisruptorPipelineOutput(recordRingBuffer)
        val feeder = ProcessorFrameFeeder(output)
        binaryDisruptor.handleEventsWith(feeder::feedDisruptor)
    ...
    private fun setupModelDisruptorChain(): List<Disruptor<*>> {
        ...
        @Suppress("UNCHECKED_CAST")
        var nextOutput = output as PipelineOutput<Any>
    ...
    private fun <T> setupModelDisruptorSegment(
        ...
        for (intermediateStage in segment.intermediateStages) {
            segmentDisruptor.handleEventsWith(EventHandler { event, _, _ ->
