 ...
 * Report → [ReportLogicCompiler]. This is the single entry point the
 * [tech.kzen.auto.server.service.impl.ServerLogicController] starts a run through, and the seam a nested
 * [tech.kzen.auto.server.exec.script.step.RunStep] / [tech.kzen.auto.server.objects.flow.vertex.RunLogicVertex]
 * compiles its child through — so the flavours nest each other uniformly (Report excepted: it is top-level only).
 *
    ...
    fun compile(
        ...
        return when {
            ...
            ReportConventions.isReport(documentNotation) ->
                ReportLogicCompiler.compile(location, graphNotation, graphDefinition, services)
