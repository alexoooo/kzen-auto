...
class MultiFileReaderWorkerTest {
        ...
        withTempDir { dir ->
            ...
            assertEquals(
                listOf(
                    ...
                    listOf("Odesa", "30"),
                    listOf("Dnipro", "40")),
                emitted.map { testRecord(it).toList() })

            // Every record carries the shared schema taken from the first file.
            assertTrue(emitted.all { testProjection(it).header == HeaderListing.of(listOf("city", "amount")) })
        }
    }
    ...
    fun synthesizesPositionalSchemaAcrossFilesWhenHeaderless() = runBlocking {
        withTempDir { dir ->
            ...
            assertTrue(emitted.all { testProjection(it).header == HeaderListing.of(listOf("c0", "c1")) })
        }
    }
    ...
    fun carriesFileCursorAcrossLiveEditSoResumeContinuesWithoutLossOrDuplication() = runBlocking {
        withTempDir { dir ->
            ...
            val emitted2 = mutableListOf<DataValue>()
            ...
            val combined = emitted1.map { testRecord(it).toList() } + emitted2.map { testRecord(it).toList() }
    ...
    private fun capturingOutput(sink: MutableList<DataValue>, batchSize: Int): ChannelOutput<Any?> =
