 ...
 * [ChannelInput] of flat-backed values and a capturing [ChannelOutput], asserting the ordering it emits at
 * end-of-stream.
 *
...
class SortWorkerTest {
    ...
    //-----------------------------------------------------------------------------------------------------------------
    private suspend fun runSort(sort: SortSpec, records: List<DataValue>): List<DataValue> {
        val forwarded = mutableListOf<DataValue>()
    ...
    private fun interruptedAfterFirstChunk(firstChunk: List<DataValue>): ChannelInput<Any?> =
        object: ChannelInput<Any?> {
            private var delivered = false
