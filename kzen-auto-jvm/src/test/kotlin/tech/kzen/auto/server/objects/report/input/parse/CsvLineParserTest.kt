...
class CsvLineParserTest {
    ...
    fun emptyValue() {
        val csvLines = ""
        assertTrue(RecordLineReader.csvLines(csvLines).isEmpty())
    }
    ...
    fun emptyQuotedValue() {
        ...
        for (i in 1 .. 2) {
            val parsed = RecordLineReader.csvLines(csvLines, i)
    ...
    fun emptyLines() {
        val csvLines = "\r\n\r\n\n\n\n\r\n"
        assertTrue(RecordLineReader.csvLines(csvLines).isEmpty())
        assertTrue(RecordLineReader.csvLines(csvLines, 1).isEmpty())
        assertTrue(RecordLineReader.csvLines(csvLines, 2).isEmpty())
        assertTrue(RecordLineReader.csvLines(csvLines, 3).isEmpty())
    }
    ...
    fun twoEmptyValues() {
        val csvLines = ","
        assertEquals(listOf("", ""), RecordLineReader.csvLines(csvLines)[0].toList())
        assertEquals(csvLines, RecordLineReader.csvLines(csvLines)[0].toCsv())
    }
    ...
    fun mannyEmptyValues() {
        val csvLines = ",,,,,,,,,"
        val values = listOf("", "", "", "", "", "", "", "", "", "")
        assertEquals(values, RecordLineReader.csvLines(csvLines)[0].toList())
        assertEquals(values, RecordLineReader.csvLines(csvLines, 1)[0].toList())
        assertEquals(values, RecordLineReader.csvLines(csvLines, 2)[0].toList())
        assertEquals(values, RecordLineReader.csvLines(csvLines, 3)[0].toList())
        assertEquals(values, RecordLineReader.csvLines(csvLines, 4)[0].toList())
    }
    ...
    fun singleCharacterValue() {
        val csvLine = "a"
        assertEquals(listOf("a"), RecordLineReader.csvLines(csvLine)[0].toList())
    }
    ...
    fun fewCharacterValue() {
        val csvLine = "abc"
        assertEquals(listOf("abc"), RecordLineReader.csvLines(csvLine)[0].toList())
        assertEquals(listOf("abc"), RecordLineReader.csvLines(csvLine, 1)[0].toList())
        assertEquals(listOf("abc"), RecordLineReader.csvLines(csvLine, 2)[0].toList())
    }
    ...
    fun twoSingleCharacterValues() {
        val csvLine = "a,b"
        assertEquals(listOf("a", "b"), RecordLineReader.csvLines(csvLine)[0].toList())
        assertEquals(listOf("a", "b"), RecordLineReader.csvLines(csvLine, 1)[0].toList())
        assertEquals(listOf("a", "b"), RecordLineReader.csvLines(csvLine, 2)[0].toList())
    }
    ...
    fun simpleBareLine() {
        ...
        assertEquals(cells, RecordLineReader.csvLines(csvLine)[0].toList())
        for (i in 1 .. 16) {
            assertEquals(cells, RecordLineReader.csvLines(csvLine, i)[0].toList())
        }
    }
    ...
    fun utf8Line() {
        ...
        val line = RecordLineReader.csvLines(csvLine)[0]
        assertEquals(25, line.toList().size)
        assertEquals(csvLine, line.toCsv())

        for (i in 1 .. 16) {
            assertEquals(csvLine, RecordLineReader.csvLines(csvLine, i)[0].toCsv())
        }
    }
    ...
    fun utf8Lines() {
        ...
        val parsed = RecordLineReader.csvLines(lines)
        assertEquals(2, parsed.size)
        assertEquals(csvLineA, parsed[0].toCsv())
        assertEquals(csvLineB, parsed[1].toCsv())

        for (i in 1 .. 128) {
            val parsedBuffered = RecordLineReader.csvLines(lines, i)
    ...
    fun multilineRecord() {
       ...
       2016 RAM 2500 Laramie Crew Cab LWB 4WD   56435bda4ec4411cbc4fa7b69190335f",,nc,36.2768,-80.3395"""
        ...
        val parsed = RecordLineReader.csvLines(csvRecord)
        assertEquals(1, parsed.size)
        assertEquals(csvRecord, parsed[0].toCsv())

        for (i in 1 .. 128) {
            val parsedBuffered = RecordLineReader.csvLines(csvRecord, i)
...
//7088746062,https://greensboro.craigslist.org/ctd/d/cary-2012-acura-tl-base-4dr-sedan/7088746062.html,greensboro,https://greensboro.craigslist.org,10299,2012,acura,tl,,,gas,90186,clean,automatic,19UUA8F22CA003926,,,other,blue,https://images.craigslist.org/01414_3LIXs9EO33z_600x450.jpg,"2012 Acura TL Base 4dr Sedan     Offered by: Best Import Auto Sales Inc — (919) 800-0650 — ${'$'}10,299     PRISTINE CONDITION INSIDE AND OUT   Best Import Auto Sales Inc    Year: 2012 Make: Acura Model: TL Series: Base 4dr Sedan VIN: 19UUA8F22CA003926  Condition: Used Mileage: 90,186  Exterior: Blue Interior: Black Body: Sedan Transmission: Automatic 6-Speed Engine: 3.5L V6      **** Best Import Auto Sales Inc. 🚘 Raleigh Auto Dealer *****  ⚡️⚡️⚡️ Call Or Text (919) 800-0650 ⚡️⚡️⚡️  ✅ - We can arrange Financing Options with most banks and credit unions!!!!     ✅ Extended Warranties Available on most vehicles!! ""Call To Inquire""  ✅ Full Service ASE-Certified Shop Onsite!       More vehicle details: best-import-auto-sales-inc.hammerwebsites.net/v/cfoamRwq     Address: 1501 Buck Jones Rd Raleigh, NC 27606   Phone: (919) 800-0650     Website: www.bestimportsonline.com      📲 ☎️ Call or text (919) 800-0650 for quick answers to your questions about this Acura TL Your message will always be answered by a real human — never an automated system.     Disclaimer: Best Import Auto Sales Inc will never sell, share, or spam your mobile number. Standard text messaging rates may apply.       2012 Acura TL Base 4dr Sedan   30b9c4702111452eb57503c99e795660",,nc,35.7636,-78.7443
//"""
//
//        val parsed = RecordLineReader.csvLines(csv)
//        assertEquals(2, parsed.size)
//    }
}