...
/**
 * Column identity includes occurrence and distinguishes the root value from a field named value. The payload's
 * columns come first, then one `meta.<name>` column per metadata field.
 */
class PreviewTable(items: List<PreviewNode>) {
    data class Column(
        ...
        val label: String get() =
            (if (metadata) "meta." else "") + name + if (occurrence <= 0) "" else " (#${occurrence + 1})"
    }
    ...
    }.plus(items.flatMap { item ->
        item.metadata?.children.orEmpty().map { Column(it.name, it.occurrence, metadata = true) }
    }).distinct().let { all ->
    ...
    val rows: List<List<PreviewNode>> = items.map { item ->
        columns.map { column ->
            if (column.metadata) {
                item.metadata?.children?.firstOrNull { it.name == column.name && it.occurrence == column.occurrence }
                    ?: absent
            }
            else if (column.root) {
