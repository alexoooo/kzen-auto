 ...
 * splits the physical batches the producer sent back into elements. A framework base Worker drains a whole
 * physical batch at a time via [receiveBatch] (so a cooperative [tech.kzen.auto.common.paradigm.job.control.JobControl.checkpoint]
 * lands per batch, not per element — one step ≈ one batch), then hands the batch's elements to its work hook one
...
interface ChannelInput<out T> {
     ...
     *   (the untyped scalar / Run lane), so a base Worker reads whole batches via [receiveBatch] to disambiguate;
    ...
    /**
     * Suspends until the next physical batch is available.
     *
     * @return the next batch of elements (never empty; individual elements may be null), or null at
     *   end-of-stream. This is the framework-facing drain: [tech.kzen.auto.server.objects.job.worker.SinkWorker]
     *   / `TransformWorker` checkpoint once per batch, then dispatch its elements to `onElement`.
     */
    suspend fun receiveBatch(): List<T>?
